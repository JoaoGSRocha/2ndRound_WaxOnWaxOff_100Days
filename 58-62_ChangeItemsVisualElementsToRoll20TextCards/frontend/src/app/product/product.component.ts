...
export class ProductComponent implements OnInit {
  ...
  addToCart(course: Skin) {
    ...
    // @ts-ignore
    this.a = [...new Set(this.course_onCart.map(item => item.stripePrice))];
    ...
    this.a.forEach(function(ele, index, theArray) {
      c.forEach(function (duplicate_ele) {
        ...
        if(duplicate_ele.stripePrice === ele)
          theArray[index] = duplicate_ele;
        if(theArray[index].quantity !== undefined)
          theArray[index].quantity = quantity;
        if(theArray[index].stripePrice === duplicate_ele.stripePrice) {
          if(theArray[index].quantity !== undefined)
  ...
  async pay(): Promise<void> {
    ...
    const prices = this.a.map(({ stripePrice, quantity }) => ({stripePrice, quantity}));
